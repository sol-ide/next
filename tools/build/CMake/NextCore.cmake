...
#                                                                        #
#   next_library_project: Defines a Next library project (e.g.,        #
#   Next.Python).                                                       #
#                                                                        #
#   next_add_library: Builds library binaries for Next libraries       #
#   with compiled sources (e.g., next_filesystem).                      #
#                                                                        #
#   next_add_executable: Builds executables.                            #
##########################################################################

# Defines a Next library project (e.g., for Next.Python). Use as:
#
#   next_library_project(libname
...
# library sources (for Next libraries that build actual library
# binaries), and testdir1, testdir2, etc, are subdirectories
# containing regression tests. DEPENDS lists the names of the other
# Next libraries that this library depends on. If the dependencies
...
#     TESTDIRS test
#     )
macro(next_library_project LIBNAME)
  ...
  string(TOLOWER "${LIBNAME}" NEXT_PROJECT_NAME)
  ...
  if (THIS_PROJECT_MODULARIZED OR THIS_PROJECT_SRCDIRS)
    ...
    if (COMMAND cpack_add_component_group)
      ...
      if (THIS_PROJECT_DESCRIPTION)
        set(THIS_PROJECT_DESCRIPTION "Next.${LIBNAME}\n\n${THIS_PROJECT_DESCRIPTION}")
  ...
  if (THIS_PROJECT_MODULARIZED)
    ...
    #
    # include_directories("${Next_SOURCE_DIR}/libs/${libname}/include")
  ...
  if (modularize_debug)
    set(modularize_output ${Next_BINARY_DIR})
    set(modularize_libs_dir "modularize")
  else (modularize_debug)
    set(modularize_output ${Next_SOURCE_DIR})
  ...
  if(THIS_PROJECT_HEADERS)
    ...
    foreach(item ${THIS_PROJECT_HEADERS})
      if(EXISTS "${Next_SOURCE_DIR}/next/${item}")
        if(IS_DIRECTORY "${Next_SOURCE_DIR}/next/${item}")
          list(APPEND ${LIBNAME}-modularize-commands
            COMMAND "${CMAKE_COMMAND}" -E copy_directory
            "${Next_SOURCE_DIR}/next/${item}"
            "${modularize_output}/${modularize_libs_dir}/${libname}/include/next/${item}"
            )
          if (NOT modularize_debug)
            list(APPEND ${LIBNAME}-modularize-commands
              COMMAND "${CMAKE_COMMAND}" -E remove_directory "${Next_SOURCE_DIR}/next/${item}"
              )
          endif (NOT modularize_debug)
        else(IS_DIRECTORY "${Next_SOURCE_DIR}/next/${item}")
          list(APPEND ${LIBNAME}-modularize-commands
          ...
          if (NOT modularize_debug)
            list(APPEND ${LIBNAME}-modularize-commands
              COMMAND "${CMAKE_COMMAND}" -E remove "${Next_SOURCE_DIR}/next/${item}"
              )
          endif (NOT modularize_debug)
          
        endif(IS_DIRECTORY "${Next_SOURCE_DIR}/next/${item}")
      elseif(EXISTS "${Next_SOURCE_DIR}/${modularize_libs_dir}/${libname}/include/next/${item}")
        # Okay; already modularized
      else()
        message(SEND_ERROR 
          "Header or directory ${Next_SOURCE_DIR}/next/${item} does not exist. The HEADERS argument in ${Next_SOURCE_DIR}/${modularize_libs_dir}/${libname}/CMakeLists.txt should be updated.")
      endif()
    endforeach(item)
    ...
    if (${LIBNAME}-modularize-commands)
      set(${LIBNAME}-modularize-commands
        ...
        "${modularize_output}/${modularize_libs_dir}/${libname}/include/next"
      ...
      # ${${LIBNAME}-modularize-commands}
      # COMMENT "Modularizing ${LIBNAME} headers to project-local dir from monolithic next dir"
      # )
  ...
  if(NOT EXISTS ${CMAKE_BINARY_DIR}/bin/tests/${NEXT_PROJECT_NAME})
    file(MAKE_DIRECTORY ${CMAKE_BINARY_DIR}/bin/tests/${NEXT_PROJECT_NAME})
  endif(NOT EXISTS ${CMAKE_BINARY_DIR}/bin/tests/${NEXT_PROJECT_NAME})
  ...
  #set(NEXT_${LIBNAME}_COMPONENTS ${THIS_PROJECT_COMPONENTS} PARENT_SCOPE)
  #message("NEXT_${LIBNAME}_COMPONENTS ${THIS_PROJECT_COMPONENTS}")
  #set(NEXT_ALL_COMPONENTS ${LIBNAME} ${NEXT_ALL_COMPONENTS} PARENT_SCOPE)
  ...
  if ((BUILD_TESTS_INDEX GREATER -1) OR (BUILD_TESTS STREQUAL "ALL"))
    ...
    if (THIS_PROJECT_TESTDIRS)
      set(NEXT_TEST_PROJECTS ${ULIBNAME} ${NEXT_TEST_PROJECTS} PARENT_SCOPE)
    endif()
    foreach(SUBDIR ${THIS_PROJECT_TESTDIRS})
      # message(STATUS "+-- ${SUBDIR}")
      # add_subdirectory(${SUBDIR})
      set(NEXT_${ULIBNAME}_TESTDIRS
	${NEXT_${ULIBNAME}_TESTDIRS}
...
endmacro(next_library_project)

macro(next_tool_project TOOLNAME)
  parse_arguments(THIS_PROJECT
  ...
  #   message(">>> ${NEXT_${UTOOLNAME}_DEPENDS}")
  #   foreach(DEP ${NEXT_${UTOOLNAME}_DEPENDS})
  #     get_target_property(dep_location next_${DEP} TYPE)
  ...
  #       #
  #       # include_directories("${Next_SOURCE_DIR}/libs/${DEP}/include")
  #       #
  #     endforeach(DEP)
  #   endif()
endmacro(next_tool_project)

#TODO: Finish this documentation
# Defines dependencies of a next project and testing targets. Use as:
#
#   next_module(libname
...
macro(next_module LIBNAME)
  ...
  # Export NEXT_${LIBNAME}_DEPENDS
  string(TOUPPER "NEXT_${LIBNAME}_DEPENDS" THIS_MODULE_LIBNAME_DEPENDS)
...
#   library target, e.g., next_signals-mt-static.
#
...
#   multi-threaded, release variant of the library in Next 1.34.0 as
...
#
macro(next_library_variant_target_name)
  set(VARIANT_TARGET_NAME "")

  # The versioned name starts with the full Next toolset
  if(WINMANGLE_LIBNAMES)
    set(VARIANT_VERSIONED_NAME "-${NEXT_TOOLSET}")
  ...
  if(WINMANGLE_LIBNAMES)
    # Append the Next version number to the versioned name
    if(NEXT_VERSION_SUBMINOR GREATER 0)
      set(VARIANT_VERSIONED_NAME
	"${VARIANT_VERSIONED_NAME}-${NEXT_VERSION_MAJOR}_${NEXT_VERSION_MINOR}_${NEXT_VERSION_SUBMINOR}")
    else(NEXT_VERSION_SUBMINOR GREATER 0)
      set(VARIANT_VERSIONED_NAME 
	"${VARIANT_VERSIONED_NAME}-${NEXT_VERSION_MAJOR}_${NEXT_VERSION_MINOR}")
    endif(NEXT_VERSION_SUBMINOR GREATER 0)
  endif(WINMANGLE_LIBNAMES)
endmacro(next_library_variant_target_name)
...
#
#   next_feature_interactions(prefix
#                              feature1 feature2 ...)
#
...
macro(next_feature_interactions PREFIX)
  ...
  if(MSVC)
    ...
    list_contains(IS_RELEASE RELEASE ${ARGN})

    set( DEBUG_RUNTIME_LIBRARY_FLAGS "" )
    set( RELEASE_RUNTIME_LIBRARY_FLAGS "" )
    if(IS_STATIC_RUNTIME)
      set( DEBUG_RUNTIME_LIBRARY_FLAGS "/MTd" )
      set( RELEASE_RUNTIME_LIBRARY_FLAGS "/MT" )
    else(IS_STATIC_RUNTIME)
      set( DEBUG_RUNTIME_LIBRARY_FLAGS "/MDd" )
      set( RELEASE_RUNTIME_LIBRARY_FLAGS "/MD" )
    endif(IS_STATIC_RUNTIME)

    if(IS_DEBUG)
      set(${PREFIX}_COMPILE_FLAGS "${DEBUG_RUNTIME_LIBRARY_FLAGS} ${${PREFIX}_COMPILE_FLAGS}")
      set( DEBUG_RUNTIME_LIBRARY_FLAGS "" )
    endif(IS_DEBUG)
    if(IS_RELEASE)
      set(${PREFIX}_COMPILE_FLAGS "${RELEASE_RUNTIME_LIBRARY_FLAGS} ${${PREFIX}_COMPILE_FLAGS}")
      set( RELEASE_RUNTIME_LIBRARY_FLAGS "" )
    endif(IS_RELEASE)

  endif(MSVC)  
endmacro(next_feature_interactions)

# This macro is an internal utility macro that builds a particular
# variant of a next library.
#
#   next_library_variant(libname 
#                         feature1 feature2 ...)
#
# where libname is the name of the Next library (e.g.,
# "next_filesystem") and feature1, feature2, ... are the features
# that will be used in this variant. 
#
# This macro will define a new library target based on libname and the
# specific variant name (see next_library_variant_target_name), which
...
macro(next_library_variant LIBNAME)
  ...
  string( TOUPPER "${LIBNAME}" THIS_ULIBNAME )
  set(THIS_VARIANT_COMPILE_FLAGS "${THIS_VARIANT_COMPILE_FLAGS} -D${THIS_ULIBNAME}_SOURCE")

  # message("next_library_variant(${LIBNAME} ${ARGN})")

  # Handle feature interactions
  next_feature_interactions("THIS_VARIANT" ${ARGN})
  next_library_variant_target_name(${ARGN})
  ...
  if (THIS_VARIANT_OKAY)
    ...
    if (THIS_LIB_IS_STATIC)
      ...
      # different names, so we follow the Next.Build version 2 style
	...
	EXPORT Next
	DESTINATION ${NEXT_LIB_INSTALL_DIR}
	COMPONENT Next) #${LIB_COMPONENT})

      # set_property( 
      #      TARGET ${VARIANT_LIBNAME}
      #      PROPERTY NEXT_CPACK_COMPONENT
      #      ${LIB_COMPONENT})
...
#     and will be extended each time ither next_add_default_variant or 
#     next_add_extra_variant is invoked. This macro will contain a list
...
#     that can be used for the next_add_library macro, separated by
#     semicolons. For example, given the use of
#     next_add_default_variant above, this variable will contain (at
#     least)
#
#        SINGLE_THREADED_COMPILE_FLAGS;SINGLE_THREADED_LINK_FLAGS;
#        MULTI_THREADED_COMPILE_FLAGS;MULTI_THREADED_LINK_FLAGS
#
#     When this variable is used in next_add_library, it turns these
...
#
#   NEXT_ADDLIB_OPTION_NAMES:
#     Like NEXT_ADD_ARG_NAMES, this variable describes
#     feature-specific options to next_add_library that can be used to
...
#
#   NEXT_ADDEXE_OPTION_NAMES:
#     Like NEXT_ADDLIB_OPTION_NAMES, except that that variable 
#     describes options to next_add_executable that can be used to
...
macro(next_add_default_variant)
  # Update NEXT_DEFAULT_VARIANTS
  if (NEXT_DEFAULT_VARIANTS)
    set(NEXT_DEFAULT_VARIANTS_ORIG ${NEXT_DEFAULT_VARIANTS})
    set(NEXT_DEFAULT_VARIANTS)
    foreach(VARIANT ${NEXT_DEFAULT_VARIANTS_ORIG})
      foreach(FEATURE ${ARGN})
        list(APPEND NEXT_DEFAULT_VARIANTS "${VARIANT}:${FEATURE}")
      endforeach(FEATURE ${ARGN})
    endforeach(VARIANT ${NEXT_DEFAULT_VARIANTS_ORIG})
    set(NEXT_DEFAULT_VARIANTS_ORIG)
  else (NEXT_DEFAULT_VARIANTS)
    set(NEXT_DEFAULT_VARIANTS ${ARGN})
  endif (NEXT_DEFAULT_VARIANTS)

  # Set Feature flag options used by the next_library macro and the
  # NEXT_FEATURES variable
  set(NEXT_DEFVAR_FEATURES)
  foreach(FEATURE ${ARGN})
    set(NEXT_ADD_ARG_NAMES 
      "${NEXT_ADD_ARG_NAMES};${FEATURE}_COMPILE_FLAGS;${FEATURE}_LINK_FLAGS;${FEATURE}_LINK_LIBS")
    set(NEXT_ADDLIB_OPTION_NAMES "${NEXT_ADDLIB_OPTION_NAMES};NO_${FEATURE}")
    set(NEXT_ADDEXE_OPTION_NAMES "${NEXT_ADDEXE_OPTION_NAMES};${FEATURE}")
    if (NEXT_DEFVAR_FEATURES)
      set(NEXT_DEFVAR_FEATURES "${NEXT_DEFVAR_FEATURES}:${FEATURE}")
    else (NEXT_DEFVAR_FEATURES)
      set(NEXT_DEFVAR_FEATURES "${FEATURE}")
    endif (NEXT_DEFVAR_FEATURES)
  endforeach(FEATURE ${ARGN})
  list(APPEND NEXT_FEATURES ${NEXT_DEFVAR_FEATURES})
endmacro(next_add_default_variant)

# Updates the set of "extra" build variants, which may be used to
# generate extra, library-specific variants of libraries.
#
#   next_add_extra_variant(feature-val1 feature-val2 ...)
#
# Each extra viarant makes it possible for libraries to define extra
# variants.  For example, writing:
# 
#    next_add_extra_variant(PYTHON_NODEBUG PYTHON_DEBUG)
#
# creates a PYTHON_NODEBUG/PYTHON_DEBUG feature pair as an extra
# variant, used by the Next.Python library, which generates separate
# variants of the Next.Python library: one variant uses the Python
# debug libraries, the other does not.
#
# The difference between next_add_default_variant and
# next_add_extra_variant is that adding a new default variant
# introduces additional variants to *all* Next libraries, unless
...
#   NEXT_ADDEXE_OPTION_NAMES:
#     See next_add_default_variant.
macro(next_add_extra_variant)
  set(NEXT_EXTVAR_FEATURES)
  foreach(FEATURE ${ARGN})
    set(NEXT_ADD_ARG_NAMES 
      "${NEXT_ADD_ARG_NAMES};${FEATURE}_COMPILE_FLAGS;${FEATURE}_LINK_FLAGS;${FEATURE}_LINK_LIBS")
    set(NEXT_ADDLIB_OPTION_NAMES "${NEXT_ADDLIB_OPTION_NAMES};NO_${FEATURE}")
    set(NEXT_ADDEXE_OPTION_NAMES "${NEXT_ADDEXE_OPTION_NAMES};${FEATURE}")
    if (NEXT_EXTVAR_FEATURES)
      set(NEXT_EXTVAR_FEATURES "${NEXT_EXTVAR_FEATURES}:${FEATURE}")
    else (NEXT_EXTVAR_FEATURES)
      set(NEXT_EXTVAR_FEATURES "${FEATURE}")
    endif (NEXT_EXTVAR_FEATURES)
  endforeach(FEATURE ${ARGN})  
  list(APPEND NEXT_FEATURES ${NEXT_EXTVAR_FEATURES})
endmacro(next_add_extra_variant)
...
# next_add_executable or next_add_library and what options the user
# has set.
macro(next_select_variant NAME PREFIX)
  set(${PREFIX}_DEBUG_AND_RELEASE FALSE)
  set(SELECT_VARIANT_OKAY TRUE)
  set(SELECT_VARIANT_FAILURE_REASONS)
  set(${PREFIX}_VARIANT)

  foreach(FEATURESET_STR ${NEXT_FEATURES})
  ...
  endforeach(FEATURESET_STR ${NEXT_FEATURES})
  ...
  if (SELECT_VARIANT_OKAY)
    ...
    # Handle feature interactions
    next_feature_interactions("${PREFIX}" ${${PREFIX}_VARIANT})
  else ()
    set(${PREFIX}_VARIANT)
  endif ()
endmacro(next_select_variant)

# Creates a new Next library target that generates a compiled library
...
#
# where libname is the name of Next library binary (e.g.,
# "next_regex") and source1, source2, etc. are the source files used
...
# root of the Next distribution, which defines the set of features
# that will be used to build Next libraries by default.
#
...
#   might want to add "-DNEXT_SIGNALS_NO_LIB=1" through this option
...
#   for libraries external to Next; for Next libraries, use DEPENDS.
#
...
#
#   DEPENDS: States that this Next library depends on and links
#   against another Next library. The arguments to DEPENDS should be
#   the unversioned name of the Next library, such as
#   "next_filesystem". Like LINK_LIBS, this option states that all
...
#   Next library against the same variant of the other Next
#   library. For example, if the next_mpi_python library DEPENDS on
#   next_python, multi-threaded variants of next_mpi_python will
#   link against multi-threaded variants of next_python.
#
...
#     tss.cpp xtime.cpp
#     SHARED_COMPILE_FLAGS "-DNEXT_THREAD_BUILD_DLL=1"
#     STATIC_COMPILE_FLAGS "-DNEXT_THREAD_BUILD_LIB=1"
#     NO_SINGLE_THREADED
#   )
macro(next_add_library SHORT_LIBNAME)
  set(LIBNAME "next_${SHORT_LIBNAME}")
  parse_arguments(THIS_LIB
    "DEPENDS;COMPILE_FLAGS;LINK_FLAGS;LINK_LIBS;EXTRA_VARIANTS;FORCE_VARIANTS;${NEXT_ADD_ARG_NAMES}"
    "MODULE;NO_INSTALL;${NEXT_ADDLIB_OPTION_NAMES}"
    ${ARGN}
    )
  ...
  #
  set(NEXT_ALL_COMPONENTS ${SHORT_LIBNAME} ${NEXT_ALL_COMPONENTS} 
    PARENT_SCOPE)
  ...
  if (THIS_LIB_EXTRA_VARIANTS)
    ...
    foreach(VARIANT ${NEXT_DEFAULT_VARIANTS})
    ...
    endforeach(VARIANT ${NEXT_DEFAULT_VARIANTS})
  else (THIS_LIB_EXTRA_VARIANTS)
    set(THIS_LIB_VARIANTS ${NEXT_DEFAULT_VARIANTS})
  ...
  foreach(VARIANT_STR ${THIS_LIB_VARIANTS})
    ...
    next_library_variant(${LIBNAME} ${VARIANT})
...
endmacro(next_add_library)

# Creates a new executable from source files.
#
#   next_add_executable(exename
#                        source1 source2 ...
#                        [COMPILE_FLAGS compileflags]
#                        [feature_COMPILE_FLAGS compileflags]
#                        [LINK_FLAGS linkflags]
...
#   next_add_executable (see the "feature" argument description,
...
#   external to Next; for Next libraries, use DEPENDS.
#
...
#   name of the Boost library, such as "next_filesystem". Like
...
#   next_add_executable, DEPENDS will ensure that we only link
...
#
# Example:
#   next_add_executable(wave cpp.cpp 
#     DEPENDS next_wave next_program_options next_filesystem 
#             next_serialization
#     )
macro(next_add_executable EXENAME)
  # Note: ARGS is here to support the use of next_add_executable in
  # the testing code.
  parse_arguments(THIS_EXE
    "DEPENDS;COMPILE_FLAGS;LINK_FLAGS;LINK_LIBS;OUTPUT_NAME;ARGS;TARGET_PREFIX;${NEXT_ADD_ARG_NAMES}"
    "NO_INSTALL;${NEXT_ADDEXE_OPTION_NAMES}"
    ${ARGN}
    )
  ...
  # next_add_executable and what options the user has set.
  next_select_variant(${EXENAME} THIS_EXE)

  # Possibly hyphenate exe's name
  if (THIS_PROJECT_IS_TOOL)
    set(THIS_EXE_NAME ${THIS_EXE_TARGET_PREFIX}${EXENAME})
  else()
    set(THIS_EXE_NAME ${NEXT_PROJECT_NAME}-${THIS_EXE_TARGET_PREFIX}${EXENAME})
  endif()
  ...
  if (THIS_EXE_DEBUG_AND_RELEASE)
    next_library_variant_target_name(RELEASE ${THIS_EXE_VARIANT})
    set(RELEASE_VARIANT_TARGET_NAME "${VARIANT_TARGET_NAME}")
    next_library_variant_target_name(DEBUG ${THIS_EXE_VARIANT})
    set(DEBUG_VARIANT_TARGET_NAME "${VARIANT_TARGET_NAME}")
  else (THIS_EXE_DEBUG_AND_RELEASE)
    next_library_variant_target_name(${THIS_EXE_VARIANT})
  ...
  foreach(LIB ${THIS_EXE_DEPENDS})
    if (LIB MATCHES ".*-.*")
      ...
      # e.g., "next_filesystem". We add on the appropriate variant
      # name(s).
      ...
      if(THIS_EXE_DEBUG_AND_RELEASE)
        message( "dependency_check( ${LIB}${RELEASE_VARIANT_TARGET_NAME} ) ")
        message( "dependency_check( ${LIB}${DEBUG_VARIANT_TARGET_NAME} ) ")
      else()
  ...
  if (THIS_EXE_VARIANT AND (NOT DEPENDENCY_FAILURES))
    ...
    if(THIS_EXE_DEBUG_AND_RELEASE)
      set_target_properties(${THIS_EXE_NAME}
        PROPERTIES
        COMPILE_FLAGS_DEBUG "${DEBUG_RUNTIME_LIBRARY_FLAGS} ${DEBUG_COMPILE_FLAGS} ${THIS_EXE_COMPILE_FLAGS}"
        COMPILE_FLAGS_RELEASE "${RELEASE_RUNTIME_LIBRARY_FLAGS} ${RELEASE_COMPILE_FLAGS} ${THIS_EXE_COMPILE_FLAGS}"
        LINK_FLAGS_DEBUG "${DEBUG_RUNTIME_LIBRARY_FLAGS} ${DEBUG_LINK_FLAGS} ${DEBUG_EXE_LINK_FLAGS} ${THIS_EXE_LINK_FLAGS}"
        LINK_FLAGS_RELEASE "${RELEASE_RUNTIME_LIBRARY_FLAGS} ${RELEASE_LINK_FLAGS} ${RELEASE_EXE_LINK_FLAGS} ${THIS_EXE_LINK_FLAGS}"
      )
    endif (THIS_EXE_DEBUG_AND_RELEASE)
...
endmacro(next_add_executable)


# Like next_add_library, but builds a single library variant
# FIXME: I'm not sure if I like this or not. Document it if it survives.
macro(next_add_single_library LIBNAME)
  parse_arguments(THIS_LIB
    "DEPENDS;COMPILE_FLAGS;LINK_FLAGS;LINK_LIBS;${NEXT_ADD_ARG_NAMES}"
    "NO_INSTALL;MODULE;${NEXT_ADDEXE_OPTION_NAMES}"
    ${ARGN}
    )
  set(THIS_LIB_SOURCES ${THIS_LIB_DEFAULT_ARGS})

  next_select_variant(${LIBNAME} THIS_LIB)
  trace(THIS_LIB_VARIANT)
  if (THIS_LIB_VARIANT)
    add_custom_target(${LIBNAME})
    separate_arguments(THIS_LIB_VARIANT)
    next_library_variant(${LIBNAME} ${THIS_LIB_VARIANT})
  endif ()
endmacro(next_add_single_library)


#
#  Macro for building next.python extensions
#
macro(next_python_extension MODULE_NAME)
