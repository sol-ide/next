...
#ifdef _MSC_VER
      const std::size_t index_of_begin_name = std::strlen( "const class std::basic_string<char,struct std::char_traits<char>,class std::allocator<char> > &__cdecl next::details::get_typename_impl<" );
			const std::size_t index_of_end_name = std::strlen( function_name ) - std::strlen( ">(void)" );
#else
#  ifdef __GNUC__
#    ifdef __clang__
            const std::size_t index_of_begin_name = std::strlen( "const std::string &next::details::get_typename_impl(" );
...
#  endif
      ...
      static const char* typename_noise_list[] = { "struct ", "class ", "struct", "class" };
